...
# -- (a) Build 5x5 Gaussian kernel manually-------------------------------
sigma = 2
ksize = 5
...
# -- (b) Build 51x51 Gaussian kernel and visualise as 3D surface----------
ksize_big = 51
...
ax_b.set_title('51x51 Gaussian Kernel (sigma=2)')
ax_b.set_xlabel('x')
ax_b.set_ylabel('y')
...
# -- (c) Apply manual 5x5 kernel using cv.filter2D------------------------
g_manual = cv.filter2D(f, -1, G5)

# -- (d) Apply cv.GaussianBlur with same sigma and compare--------------------
...
ax[1].set_title('Manual Gaussian (5x5, sigma=2)\ncv.filter2D')

ax[2].imshow(g_cv, cmap='gray', vmin=0, vmax=255)
ax[2].set_title('cv.GaussianBlur (5x5, sigma=2)')

...
# g = f^gamma,  f in [0,1]   (gamma < 1 brightens: expands dark pixel range)
gamma = 0.5
t = np.array([(i/255.0)**(gamma)*255 for i in np.arange(0,256)]).astype(np.uint8)
g_05 = cv.LUT(f, t)

# -- (b) Gamma correction  gamma = 2.0----------------------------------------
# gamma > 1 darkens: compresses dark pixel range
gamma = 2.0
t = np.array([(i/255.0)**(gamma)*255 for i in np.arange(0,256)]).astype(np.uint8)
g_2 = cv.LUT(f, t)
...
ax[1].set_title('Gamma Correction gamma=0.5')

ax[2].imshow(g_2, cmap='gray', vmin=0, vmax=255)
ax[2].set_title('Gamma Correction gamma=2')
